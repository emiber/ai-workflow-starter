...
1. Read `workflow/rules.md`, `workflow/principles.md`, `workflow/issue-creation.md`, and `workflow/issue-tracker.md`. This is a pre-implementation task → the planning model (Opus) is fine; if you're not on `opusplan`, `/model opus`.
...
4. Draft a concise issue (short imperative title + a body with the problem and known context). Show it and confirm before creating.
5. Create it with the tracker command (`gh issue create` / `jira issue create`).
6. Confirm with the issue number/key and URL, and suggest `/refine-issue <n>` to make it implementable.
