---
name: review-pr
description: Review an open PR against its linked issue's acceptance criteria and the Definition of Done. Use when asked to review a pull request. Read-only. Mirrors the Claude Code /review-pr command; logic lives in workflow/.
---

# review-pr

Review the target open PR (its number — provided as the argument, or stated by the user) against its linked issue and the Definition of Done, following the contract in `workflow/pr-review.md`. This is **read-only**: never commit, push, or modify files.
