...
3. Generate:
   ...
   - `.github/workflows/ci.yml` filled in for the chosen stack: on every PR to `main`, run lint + tests and fail if coverage is below the threshold in `docs/ARCHITECTURE.md` (default 85%). Replace the placeholder guard step that ships with the template with real setup/install/lint/test steps, and rename the workflow back to `name: CI` and the job to a real name.
