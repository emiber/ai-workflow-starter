...
- Never `git commit` or `git push` unless the user explicitly asks. Don't commit or push on your own initiative.
- Never commit secrets or credentials. Keep them in gitignored `.env` files; provide a `.env.example` with placeholders.
